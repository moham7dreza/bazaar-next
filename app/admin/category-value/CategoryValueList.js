      ...
      <table className="min-w-full bg-white border border-gray-300 rounded-md shadow-md">
        <thead className="bg-gray-100">
          <tr>
            ...
            <th className="border px-4 py-2 text-right">مقدار</th>
            <th className="border px-4 py-2 text-right">ویژگی</th>
        ...
        <tbody>
          {categoryValues?.data.map((categoryValue) => {
            return (
              <tr key={categoryValue.id} className="hover:bg-gray-50">
                ...
                <td className="border px-4 py-2 text-right">
                  {categoryValue.categoryAttribute.name}
                </td>
                {/* <td className="border px-4 py-2 text-right">
                  {categoryAttribute.type ? "فعال" : "غیرفعال"}
                </td> */}
                <td className="border px-4 py-2 text-right">
                  {categoryValue.status ? "فعال" : "غیرفعال"}
                </td>
                <td className="border px-4 py-2 text-right">
                  {converterToJalali(categoryValue.created_at)}
                </td>
                <td className="border px-4 py-2 text-right">
                  <Link
                    href={`/admin/category-value/edit/${categoryValue.id}`}
                    className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded mx-2"
                  >
                    <i className="fa fa-edit"></i>
                  </Link>
                  <button
                    onClick={() => handleDelete(categoryValue.id)}
