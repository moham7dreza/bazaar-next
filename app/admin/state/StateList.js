...
const StateList = ({ states }) => {
  ...
  const getParentName = (parentId) => {
    const parent = states.data.find((cat) => cat.id === parentId);
      ...
      <table className="min-w-full bg-white border border-gray-300 rounded-md shadow-md">
        <thead className="bg-gray-100">
          <tr>
            ...
            <th className="border px-4 py-2 text-right">منطقه پدر</th>
          ...
          {states?.data.map((state) => {
            return (
              <tr key={state.id} className="hover:bg-gray-50">
                ...
                <td className="border px-4 py-2 text-right">{state.name}</td>
                <td className="border px-4 py-2 text-right">
                  {getParentName(state.parent_id)}
                </td>
                <td className="border px-4 py-2 text-right">
                  {state.description}
                </td>
