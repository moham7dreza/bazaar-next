...
const AdEditForm = ({ ad }) => {
  ...
  const [adsStatus, setAdsStatus] = useState(ad.ads_status || "");
  const [categoryId, setCategoryId] = useState(
    ad.category_id?.toString() || ad.category?.id?.toString() || ""
  );
  const [cityId, setCityId] = useState(
    ad.city_id?.toString() || ad.city?.id?.toString() || ""
  );
  const [userId, setUserId] = useState(
    ad.user_id?.toString() || ad.user?.id?.toString() || ""
  );
  const [publishedAt, setPublishedAt] = useState(
    ad.published_at ? ad.published_at.split("T")[0] : ""
  );
  const [expiredAt, setExpiredAt] = useState(
    ad.expired_at ? ad.expired_at.split("T")[0] : ""
  );
  const [contact, setContact] = useState(ad.contact || "");
  const [isSpecial, setIsSpecial] = useState(ad.is_special?.toString() || "0");
  const [image, setImage] = useState(null);
  const [imagePreview, setImagePreview] = useState(null);
  const [tags, setTags] = useState(ad.tags || "");
  const [lng, setLng] = useState(ad.lng || "");
  const [lat, setLat] = useState(ad.lat || "");
  const [willingToTrade, setWillingToTrade] = useState(
    ad.willing_to_trade?.toString() || "0"
  );
  const [price, setPrice] = useState(ad.price || "");
  const [status, setStatus] = useState(ad.status?.toString() || "1");
  ...
  // Update form values when ad data is available
  useEffect(() => {
    console.log("Ad data:", ad);
    if (ad) {
      setTitle(ad.title || "");
      setDescription(ad.description || "");
      setAdsType(ad.ads_type || "");
      setAdsStatus(ad.ads_status || "");
      setCategoryId(
        ad.category_id?.toString() || ad.category?.id?.toString() || ""
      );
      setCityId(ad.city_id?.toString() || ad.city?.id?.toString() || "");
      setUserId(ad.user_id?.toString() || ad.user?.id?.toString() || "");
      setPublishedAt(ad.published_at ? ad.published_at.split("T")[0] : "");
      setExpiredAt(ad.expired_at ? ad.expired_at.split("T")[0] : "");
      setContact(ad.contact || "");
      setIsSpecial(ad.is_special?.toString() || "0");
      setTags(ad.tags || "");
      setLng(ad.lng || "");
      setLat(ad.lat || "");
      setWillingToTrade(ad.willing_to_trade?.toString() || "0");
      setPrice(ad.price || "");
      setStatus(ad.status?.toString() || "1");
    }
  }, [ad]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/admin/advertisements/advertisement/${ad.id}`,
        {
          method: "POST",
          headers: {
